...
public class GUI_QLPHONG extends JFrame{
	...
	private static DefaultTableModel modelPhong, modelLoai;
	...
	public GUI_QLPHONG(JPanel pnQLPhong, JTabbedPane tabbedPane) throws ClassNotFoundException {
...
//		1
		...
		String[] headPhong = {"SỐ PHÒNG", "MÃ LOẠI PHÒNG", "TÌNH TRẠNG"};
		JScrollPane scrollpanePhong;
		pnPhongTable.add(scrollpanePhong = new JScrollPane(tablePhong = new JTable(modelPhong = new DefaultTableModel(headPhong, 0)),
		...
		String[] headLoaiPhong = {"MÃ LOẠI PHÒNG", "SỐ KHÁCH TỐI ĐA", "ĐƠN GIÁ"};
		JScrollPane scrollpaneLoaiPhong;		
		pnLoaiPhongTable.add(scrollpaneLoaiPhong = new JScrollPane(tableLoai = new JTable(modelLoai = new DefaultTableModel(headLoaiPhong, 0)),
	...
	public void DocPhong() {
		List<PHONG> listPhong = daoP.getAlltbPhong();
		for (PHONG p : listPhong) {
			modelPhong.addRow(new Object[] { p.getSoPhong(), p.getPhong().getMaLoaiPhong(), p.getTinhTrang()});						
		}
	}
	public void DocLoaiPhong() {
		List<LOAIPHONG> listLP = daoLP.getalltbLoaiPhong();		
		for(LOAIPHONG lp : listLP) {				
			modelLoai.addRow(new Object[] {lp.getMaLoaiPhong(), lp.getSoLuong(), lp.getDonGia()});					
			try {
				lblImg.setIcon(new ImageIcon(lp.getHinhAnh()));
			}catch (Exception e) {
